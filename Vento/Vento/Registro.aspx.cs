...
namespace Vento
{
    public partial class Registro : System.Web.UI.Page
    {
        ...
        public static string GetOcupations(string hint)
        {
            List<string> result = new List<string>();
            StringBuilder sb = new StringBuilder();
            string sConection = WebConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString;
            ...
            using (SqlConnection sqlCon = new SqlConnection(sConection))
            {
                SqlCommand sqlCom = new SqlCommand("select ocupation from ocupations where ocupation like '%" + hint + "%'  order by ocupation asc", sqlCon);
                sqlCon.Open();
                SqlDataReader dr = sqlCom.ExecuteReader();
                while (dr.Read() && (limit++) < 5)
                {
                    sb.Append("<span>" + dr["ocupation"].ToString() + "</span>");
                }
                sqlCon.Close();
            }
            return sb.ToString();
                ...
                try
                {
                    ...
                    if (register)
                    {
                        if (choice.SelectedIndex == 1)
                        {
                            ...
                            {
                                SqlCommand sqlCom001 = new SqlCommand("select count(id) as total from users_registered where name='" + txtNombre.Text + "'", sqlCon001);
                                sqlCon001.Open();
                            ...
                            if (counter_record < 1 && txtResidencia.Text != "" && txtOcupacion.Text != "")
                            {
                                //revisar localidad y ocupacion
                                int counter_localidad = 0;
                                int counter_ocupacion = 0;
                                int id_localidad = 0;
                                int id_ocupacion = 0;
                                ...
                                #endregion
                                #region ocupacion
                                string sConection01 = WebConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString;
                                using (SqlConnection sqlCon01 = new SqlConnection(sConection01))
                                {
                                    SqlCommand sqlCom01 = new SqlCommand("select count(ocupations.ocupation) as total from ocupations where ocupations.ocupation='" + txtOcupacion.Text + "'", sqlCon01);
                                    sqlCon01.Open();
                                    SqlDataReader reader01 = sqlCom01.ExecuteReader();
                                    while (reader01.Read())
                                    {
                                        counter_ocupacion = Convert.ToInt16(reader01[0]);
                                    }
                                    sqlCon01.Close();
                                }
                                if (counter_ocupacion < 1)
                                {
                                    string sConection1 = WebConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString;
                                    using (SqlConnection sqlCon = new SqlConnection(sConection1))
                                    {
                                        SqlCommand sqlCom = new SqlCommand("INSERT INTO ocupations(ocupation) OUTPUT inserted.id VALUES ('" + txtOcupacion.Text + "')", sqlCon);
                                        sqlCon.Open();
                                        SqlDataReader reader = sqlCom.ExecuteReader();
                                        while (reader.Read())
                                        {
                                            id_ocupacion = Convert.ToInt32(reader[0]);
                                        }
                                        sqlCon.Close();
                                    }
                                }
                                else
                                {
                                    ...
                                    using (SqlConnection sqlCon1 = new SqlConnection(sConection2))
                                    {
                                        SqlCommand sqlCom1 = new SqlCommand("select id from ocupations where ocupation ='" + txtOcupacion.Text + "'", sqlCon1);
                                        sqlCon1.Open();
                                        SqlDataReader reader1 = sqlCom1.ExecuteReader();
                                        while (reader1.Read())
                                        {
                                            id_ocupacion = Convert.ToInt32(reader1[0]);
                                        }
                                        sqlCon1.Close();
                                    }
                                }
                                ...
                                {
                                    SqlCommand sqlComu = new SqlCommand("insert into users_registered(name,age,residence,ocupacion_id,experience,email,phone) values ('" + txtNombre.Text + "','" + edad.ToString() + "','" + txtResidencia.Text + "'," + id_ocupacion + ",'" + txtExperiencia.Text + "','" + txtEmail.Text + "','" + txtTelefono.Text + "')", sqlConu);
                                    sqlConu.Open();
